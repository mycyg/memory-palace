...
準備中やキューにあるコンテキストは容量だけを予約します。同じ本文への有効な `accepted` 受領で使用済み予算に算入し、破棄が確認された場合は予約を解放します。バックグラウンドジョブがタイムアウトしても、ローカルの呼び出しが終わるまで実行枠を占有します。
...
バックアップには SQLite スナップショットが参照する添付ファイルだけを含めます。復元は隔離した準備用ディレクトリでデータベースと添付ファイルを検証し、準備後に空の対象へ反映します。
