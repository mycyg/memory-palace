...
from eventmem.core.models import Scope, ReminderPolicy, ReminderInput
...
def test_outbox_crash_claim_uncertainty_and_pause_resume(tmp_path, monkeypatch):
    ...
    second = s.schedule(
        ReminderInput(command_id="second", record_id=rid, due_at="2020-01-01T00:00:00Z")
    )
...
def test_process_death_after_remote_effect_leaves_uncertain_delivery(tmp_path):
    ...
    s.schedule(
        ReminderInput(command_id="crash", record_id=rid, due_at="2020-01-01T00:00:00Z")
    )
...
def test_uncertain_acknowledgment_advances_only_its_occurrence(tmp_path):
    ...
    rid = e.source(source["id"])["record_ids"][0]
    s.policy(ReminderPolicy(quiet_start=0, quiet_end=0))
    schedule = s.schedule(
        ReminderInput(
            command_id="daily",
            record_id=rid,
    ...
    one = s.schedule(
        ReminderInput(command_id="cancel", record_id=rid, due_at="2020-01-01T00:00:00Z")
    )
    delivery = s.tick(deliver=False)["created"][0]
    with e.db.connect(write=True) as c:
        c.execute("UPDATE outbox SET state='uncertain' WHERE id=?", (delivery,))
